...
import { getCodeList } from "@/app/api/getInfoRac";
import { formatDate } from "date-fns";
import { ScrollArea } from "../ui/scroll-area";
import { Circle, Squircle, SquircleIcon } from "lucide-react";
...
export function CodeListPage() {
  ...
  const searchInTypeNomina = (tipoNomina: string) => {
    if (Array.isArray(originCodeList.data)) {
      const filteredCodes = originCodeList.data.filter((item) =>
        item.tiponomina.nomina.toLowerCase().includes(tipoNomina.toLowerCase())
      );
      setCodeList({ ...originCodeList, data: filteredCodes });
      return;
    }
