...
#         if self.instance:
#             if self.instance.cedulaidentidad != value:
#                 raise serializers.ValidationError(
#                     "La cedula es un campo permanente y no se puede modificar"
#                 )
#         return value
#     def to_internal_value(self, data):
        
#         # ataja los datos y verifica todos las llaves foraneas, si llegan en 0 los transforma a none 
#         data = data.copy() if hasattr(data, 'copy') else data.copy()

#         fk_fields = [
#             "tallaCamisa", "tallaPantalon", "tallaZapatos", 
#             "nivelAcademico", "estadoCivil","grupoSanguineo", "estadoid", 
#             "municipioid", "parroquiaid", "discapacidad"
#         ]
...
# # transforma las fechas si llegan en formato ISO 
#         date_fields = [
#             'fecha_nacimiento',
...
# # ataja los nombres y apellidos y los transforma en mayuscula
...
#     def validate_direccionExacta(self, value):
#         return value.upper() if value else value
    
#     def create(self, validated_data):
#         # toma el id del usuario para asignarlo en el historial 
#         id_usuario = validated_data.pop('usuario_id')
#         # procesa los forekeys de patologias cronicas 
#         patologias = validated_data.pop('patologiaCronica', [])
#         try:
#           usuario_obj = User.objects.get(pk=id_usuario)
#         except User.DoesNotExist:
#           raise serializers.ValidationError({"El usuario no existe"})
...
#         if patologias:
#           instance.patologiaCronica.set(patologias)

#         return instance
       
#     #    Actualizacion de datos (toma el id del usuario para asignarlo en el historial)
#     def update(self, instance, validated_data):
...
class EmployeeSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if salud_data:
            perfil_salud.objects.update_or_create(empleado_id=instance, defaults=salud_data)
...
class EmployeeCargoSerializer(serializers.ModelSerializer):
    ...
    salud_info = serializers.SerializerMethodField()
    fisico_info = serializers.SerializerMethodField()
    ...
    def get_salud_info(self, obj):
        ...
        return {
            ...
            "discapacidad": DiscapacidadSerializer(salud.discapacidad).data,
            "patologiasCronicas": PatologiasSerializer(salud.patologiaCronica, many=True).data
        }

    def get_fisico_info(self, obj):
...
class EmployeeListarDataSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'sexo', 'estadoCivil', 'vivienda_info', 'salud_info', 
            'fisico_info', 'formacion_info', 'antecedentes',
            'fecha_actualizacion'
        ]
