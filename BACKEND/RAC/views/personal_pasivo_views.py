...
@extend_schema(
    tags=["Gestion de Personal Pasivo"],
    methods=['POST'],
    summary="Migrar Familiar a Pensionado Sobreviviente",
    description="Procesa la migración de un familiar heredero, creando un nuevo registro como pensionado sobreviviente",
    request=MigracionSobrevivienteSerializer,
)
@api_view(['POST'])
def migrar_heredero(request):
    serializer = MigracionSobrevivienteSerializer(data=request.data)
    
    if serializer.is_valid():
        try:
            empleados_creados = serializer.save() 
            return Response({
                'status': "success",
                'message': "Pensiones de sobreviviente  registradas con éxito",
                'data': empleados_creados
            }, status=status.HTTP_201_CREATED)
    ...
    try:
        ...
        first_error_value = list(errors_dict.values())[0]
        if isinstance(first_error_value, list) and isinstance(first_error_value[0], dict):
             first_error = list(first_error_value[0].values())[0][0]
        elif isinstance(first_error_value, list):
             first_error = first_error_value[0]
        else:
             first_error = str(first_error_value)
    except:
        first_error = "Error de validación en los datos enviados"
